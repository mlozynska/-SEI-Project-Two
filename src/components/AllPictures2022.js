...
const AllPictures2022 = () => {
  ...
  const getData = async () => {
    try {
      const { data } = await axios.get(`https://api.nasa.gov/planetary/apod?api_key=dg5GyW52DZwLxZIT4MpUfvRa01eHvDU3sbgf94zE&start_date=2022-${startMonth}-01&end_date=${endDate}`)
      setStars(data)
      setNewStars(data)
    } catch (err) {
      setHasError(true)
