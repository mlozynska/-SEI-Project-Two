    ...
    try {
      const getData = async () => {
        const { data } = await axios.get(
          `https://api.nasa.gov/planetary/apod?api_key=etjYr41gKjN2Wh8ThofDOj0HJI3i43FjgGX4lh0g&date=${yourdate}`
        )
        setYourStar(data)
      }
      getData()
            ...
            <div className="container">
              <div>
                <h2 className="title has-text-centered">
                  {yourStar.media_type === 'video'
                    ? 'We are sorry, but there is no picture for this day.'
                    : 'We are getting your picture...'}
                </h2>
                <hr />
              </div>
            </div>
